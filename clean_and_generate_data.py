...
df_hospitals['capacity'] = [np.random.randint(20, 500) for _ in range(len(df_hospitals))]
df_hospitals['emergency_facility'] = [np.random.choice(['True', 'False']) for _ in range(len(df_hospitals))]
...
def generate_appointments(n):
    ...
    for i in tqdm(range(n), desc="Generating appointments"):
        idx_p = np.random.randint(0, len(patient_ids) - 1)
        idx_d = np.random.randint(0, len(doctor_ids) - 1)

        data.append({
            'appointment_id': 'APP{:06d}'.format(i),
...
df_appointments.to_csv(f"{DATA_PROCESSED}/appointments.csv",
                       index=False)
...
def generate_diagnosis(n):
    ...
    registration_dates = pd.to_datetime(df_patients['registration_date']).values

    data = []
    for i in tqdm(range(n), desc="Generating diagnosis"):
        idx = np.random.randint(0, len(patient_ids) - 1)
        pid = patient_ids[idx]
        ...
        data.append({
            'diagnosis_id': 'DIAGNO{:05d}'.format(i),
            'patient_id': pid,
            'disease': disease,
...
df_emergency_cases.to_csv(f"{DATA_PROCESSED}/emergency_cases.csv",
                          index=False)
...
insurance_company = [
    ...
    "Religare Health Insurance (now Care Health)",
...
def generate_insurances(n):
    ...
    for i in tqdm(range(n), desc="Generating insurance data"):
        data.append({
            'insurance_id': 'INSURE{:04d}'.format(i),
            'company_name': np.random.choice(insurance_company),
            'coverage_amount': round(np.random.uniform(1000000, 10000000), 2),
...
df_insurances.to_csv(f"{DATA_PROCESSED}/insurances.csv",
                     index=False)


### -----------------------------------------------
# Assign insurance ID to only some patients
...
def assign_insurance_to_some(df_patients, df_insurances, coverage=0.7):
    ...
    # Select insurance IDs for insured patients
    selected_insurance_ids = random.sample(
        list(df_insurances['insurance_id']),
        n_insured if n_insured <= len(df_insurances) else len(df_insurances)
    )

    # Fill the insurance list with selected IDs + Nones
    insurance_list = selected_insurance_ids + [None] * (n_patients - len(selected_insurance_ids))
    np.random.shuffle(insurance_list)  # Mix insured and uninsured np.randomly

    # Assign to patient DataFrame
...
df_patients = assign_insurance_to_some(df_patients, df_insurances, coverage=0.7)
...
df_churn_label.to_csv(f"{DATA_PROCESSED}/churn_label.csv",
                      index=False)
